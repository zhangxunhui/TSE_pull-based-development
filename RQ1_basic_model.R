...
# split by 90% and 10%, 10% for latest prs for testing
test_num = round(dim(selected_df)[1] * 0.1)
test_records = selected_df[1:test_num,]
train_records = selected_df[(test_num+1):dim(selected_df)[1],]

print(paste("selected_df rows: ", nrow(selected_df), sep=""))
print(paste("test rows: ", nrow(test_records), sep=""))
print(paste("train rows: ", nrow(train_records), sep=""))
...
basic_model <- glmer(formula=formula
            , family = binomial,
            verbose=TRUE,
            data=train_records,
            control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
save(basic_model, file=model_path)
print("finish build up basic model with standardized independent factors")