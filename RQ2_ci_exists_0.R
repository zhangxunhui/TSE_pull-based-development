...
formula_pull_request = "lifetime_minutes+num_commits+src_churn+files_added+files_deleted+files_changed+friday_effect+reopen_or_not+commits_on_files_touched+hash_tag+test_inclusion+description_length+has_comments+num_comments+other_comment+comment_conflict+test_churn"
...
model5_ci_exists_0 <- glmer(formula=formula
            , family = binomial,
            verbose=TRUE,
            data=selected_df,
            control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
save(model5_ci_exists_0, file=model_path)
print("finish build up model5_ci_exists_0")