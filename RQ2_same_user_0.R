...
selected_df[['merged_or_not']] = as.factor(selected_df[['merged_or_not']]) # do not convert to logical
...
model5_same_user_0 <- glmer(formula=formula
            , family = binomial,
            verbose=TRUE,
            data=selected_df,
            control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
save(model5_same_user_0, file=model_path)
print("finish build up model5_same_user_0")
