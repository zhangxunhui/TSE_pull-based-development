...
selected_df[['merged_or_not']] = as.factor(selected_df[['merged_or_not']]) # do not 
...
model_path = "has_comments_0.RData"
print(model_path)
...
model5_has_comments_0 <- glmer(formula=formula
            , family = binomial,
            verbose=TRUE,
            data=selected_df,
            control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
save(model5_has_comments_0, file=model_path)
print("finish build up model5_has_comments_0")
