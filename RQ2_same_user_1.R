...
df = rmout(df)
selected_df = df[c(keep_factors, "merged_or_not", "project_id", "creator_id", "last_closer_id", "last_close_time")]
...
formula_inte_part = "prior_review_num"
...
formula = paste("merged_or_not~", paste(c(formula_pull_request, formula_contrib_part, formula_inte_part, formula_project_part), collapse="+"), sep="")
formula = paste(formula, "+(1|project_id)", sep="")

model_path = "same_user_1.RData"
print(model_path)
...
model5_same_user_1 <- glmer(formula=formula
            , family = binomial,
            verbose=TRUE,
            data=selected_df,
            control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
save(model5_same_user_1, file=model_path)
print("finish build up model5_same_user_1")
